local config = { 
	["avoid_injury"] = { 
		k="avoid_injury",
		value="30#5#4#0#0#3#20#0#0#15#16#16#18#15#15",
	},
	["transaction_limit"] = { 
		k="transaction_limit",
		value="1#1000020#0|2#1000030#0|5#1000040#0|8#0#300001",
	},
	["team_num"] = { 
		k="team_num",
		value=10,
	},
	["guaiwugongcheng"] = { 
		k="guaiwugongcheng",
		value="3#30",
	},
	["guaiwugongcheng_1"] = { 
		k="guaiwugongcheng_1",
		value="296#335|160",
	},
	["guaiwugongcheng_2"] = { 
		k="guaiwugongcheng_2",
		value="蝎子王|雪蚕王|蛤蟆王|电僵王|楔蛾王|暗之触龙神|半兽勇士9|邪恶钳虫2|红野猪3|黑野猪3|蝎蛇3|幻影蜘蛛|血金刚|带刀护卫|赤血恶魔|灰血恶魔|赤月恶魔1|暴牙蜘蛛|暴牙蜘蛛0|血僵尸",
	},
	["jinzhusancai"] = { 
		k="jinzhusancai",
		value="5|夕",
	},
	["union_shop_limit"] = { 
		k="union_shop_limit",
		value=300,
	},
	["union_shop_flsh"] = { 
		k="union_shop_flsh",
		value=0,
	},
	["union_shop_announce"] = { 
		k="union_shop_announce",
		value="1#5|7#8",
	},
	["guild_updata"] = { 
		k="guild_updata",
		value="1#2#0|2#100#9999|3#150#99999",
	},
	["gold_guildexp"] = { 
		k="gold_guildexp",
		value="1000#1|1000#10|1000000",
	},
	["Found_Faction"] = { 
		k="Found_Faction",
		value="118#1#17|1#1000000#27",
	},
	["GUILD_limit"] = { 
		k="GUILD_limit",
		value="38|42|46|50|55",
	},
	["announce"] = { 
		k="announce",
		value="1#传奇3怀旧服:\\n 正版授权 长久稳定\\n 旷世巨作 震撼来袭\\n 守护传奇3最后一片净土\\n 试着让我们的故事继续！\\n 官网:www.996m2.com",
	},
	["recruitment"] = { 
		k="recruitment",
		value="1#招募公告:\\n 正版授权 长久稳定\\n 旷世巨作 震撼来袭\\n 守护传奇3最后一片净土\\n 试着让我们的故事继续！\\n 官网:www.996m2.com",
	},
	["GUILD_EXIT_CD"] = { 
		k="GUILD_EXIT_CD",
		value=3600,
	},
	["union_warehouse_time"] = { 
		k="union_warehouse_time",
		value="4#180&5#240&6#360&7#1720",
	},
	["consignments_shelves"] = { 
		k="consignments_shelves",
		value=8,
	},
	["consignments_putaway_time"] = { 
		k="consignments_putaway_time",
		value=86400,
	},
	["consignments_putaway_pay"] = { 
		k="consignments_putaway_pay",
		value=10000,
	},
	["consignments_putaway_need"] = { 
		k="consignments_putaway_need",
		value=100020,
	},
	["consignments_putaway_LEVEL"] = { 
		k="consignments_putaway_LEVEL",
		value=30,
	},
	["consignments_putaway_price_floor"] = { 
		k="consignments_putaway_price_floor",
		value="1#100000",
	},
	["consignments_putaway_price_gold"] = { 
		k="consignments_putaway_price_gold",
		value="10000#100000000",
	},
	["consignments_service_charge"] = { 
		k="consignments_service_charge",
		value=10,
	},
	["consignments_service_charge_floor"] = { 
		k="consignments_service_charge_floor",
		value=1,
	},
	["consignments_service_charge_Upper"] = { 
		k="consignments_service_charge_Upper",
		value=99999999,
	},
	["consignments_pay_need"] = { 
		k="consignments_pay_need",
	},
	["consignments_pay_Term"] = { 
		k="consignments_pay_Term",
		value="1#30#0;2#30#0;7#40#0;9#50#0;21#60#0;43#70#0;73#80#0",
	},
	["exp_max"] = { 
		k="exp_max",
		value=2000000000,
	},
	["autousetimes"] = { 
		k="autousetimes",
		value=5,
	},
	["auto_equip_quality"] = { 
		k="auto_equip_quality",
		value=3,
	},
	["warehouse_max_num"] = { 
		k="warehouse_max_num",
		value=168,
	},
	["warehouse_num"] = { 
		k="warehouse_num",
		value=38,
	},
	["warehouse_expansion"] = { 
		k="warehouse_expansion",
		value="279#1#8",
	},
	["currency_shield"] = { 
		k="currency_shield",
		value="10|14",
	},
	["cangbaotufanwei"] = { 
		k="cangbaotufanwei",
		value=5,
	},
	["cangbaotu_item_Announce_1"] = { 
		k="cangbaotu_item_Announce_1",
		value="380|149#150",
	},
	["cangbaotu_item_Announce_2"] = { 
		k="cangbaotu_item_Announce_2",
		value="381|1005302",
	},
	["cangbaotu_item_Announce_3"] = { 
		k="cangbaotu_item_Announce_3",
		value="382|1005302",
	},
	["cangbaotu_item_Announce_4"] = { 
		k="cangbaotu_item_Announce_4",
		value="383|1015730#1019730#1026730#1022730#1064730#1062730#1115730#1119730#1126730#1122730#1164730#1162730#1106730#1215730#1219730#1226730#1222730#1264730#1262730",
	},
	["cangbaotu_boss_position_1"] = { 
		k="cangbaotu_boss_position_1",
		value="14#15",
	},
	["cangbaotu_boss_position_2"] = { 
		k="cangbaotu_boss_position_2",
		value="14#15",
	},
	["cangbaotu_boss_position_3"] = { 
		k="cangbaotu_boss_position_3",
		value="11#11|14#11|17#11|11#14|14#14|17#14|11#17|14#17|17#17|11#20|14#20|17#20",
	},
	["cangbaotu_mapid"] = { 
		k="cangbaotu_mapid",
		value="0|3|11|4",
	},
	["cangbaotu_key"] = { 
		k="cangbaotu_key",
		value=30,
	},
	["cangbaotu_backroom_reward"] = { 
		k="cangbaotu_backroom_reward",
		value="248&10002#10002#10002#10002#10003#10003#10003#10003#10003#10004#10004#10005|249&10006#10006#10006#10006#10006#10007#10007#10007#10007#10007#10007#10008",
	},
	["cangbaotu_caijitime"] = { 
		k="cangbaotu_caijitime",
		value=5,
	},
	["paihangbang_title_1"] = { 
		k="paihangbang_title_1",
		value="384|385|386",
	},
	["level_max"] = { 
		k="level_max",
		value=200,
	},
	["Elite_Challenge_time"] = { 
		k="Elite_Challenge_time",
		value=3,
	},
	["Elite_DayChallenge_time"] = { 
		k="Elite_DayChallenge_time",
		value=3,
	},
	["auto_task_time"] = { 
		k="auto_task_time",
		value=500,
	},
	["Elite_DayChallenge_consumption"] = { 
		k="Elite_DayChallenge_consumption",
		value=304,
	},
	["chuanyin_item"] = { 
		k="chuanyin_item",
		value=302,
	},
	["Elite_DayChallenge_Starprobability"] = { 
		k="Elite_DayChallenge_Starprobability",
		value="70#25#5",
	},
	["Maincity_limit"] = { 
		k="Maincity_limit",
		value="6#2300001|bsr03#2300002",
	},
	["zhuizongcost"] = { 
		k="zhuizongcost",
		value="321#1|3#2",
	},
	["fridndnumberlimit"] = { 
		k="fridndnumberlimit",
		value=100,
	},
	["declareWar"] = { 
		k="declareWar",
		value="2#1#100000&4#1#200000&8#1#300000&12#1#500000",
	},
	["declareWar_time"] = { 
		k="declareWar_time",
		value="2#4#8#12",
	},
	["alliance"] = { 
		k="alliance",
	},
	["alliance_time"] = { 
		k="alliance_time",
	},
	["noDigMonsters"] = { 
		k="noDigMonsters",
		value="1#2#3",
	},
	["drug_tips"] = { 
		k="drug_tips",
		value="<普通红药：/FCOLOR=255><金创药/FCOLOR=251>\\<普通蓝药：/FCOLOR=255><魔法药/FCOLOR=251>\\<瞬回药：/FCOLOR=255><万年雪霜/FCOLOR=251>",
	},
	["boxtexiao"] = { 
		k="boxtexiao",
		value="15#4231#4211#4212|16#4231#4213#4214|17#4232#4215#4216|18#4233#4217#4218|18#4230#4219#4220|",
	},
	["attackglobalCD"] = { 
		k="attackglobalCD",
		value=1120,
	},
	["magicglobalCD"] = { 
		k="magicglobalCD",
		value=1250,
	},
	["HumanPaperback"] = { 
		k="HumanPaperback",
		value="6#32|7#31|8#33",
	},
	["MonsterPaperback"] = { 
		k="MonsterPaperback",
		value=40,
	},
	["BuiltinCD"] = { 
		k="BuiltinCD",
		value="3000#3000#500#2000",
	},
	["buttonSmall"] = { 
		k="buttonSmall",
		value=3,
	},
	["StallName"] = { 
		k="StallName",
		value="<$USERNAME>的摊位",
	},
	["BackpackGuide"] = { 
		k="BackpackGuide",
		value="1#1#41|42|43|44",
	},
	["Fashionfx"] = { 
		k="Fashionfx",
		value=0,
	},
	["SuitCalcType"] = { 
		k="SuitCalcType",
		value=0,
	},
	["Hangxuan"] = { 
		k="Hangxuan",
		value=0,
	},
	["RankingList"] = { 
		k="RankingList",
		value="1#等\\n级|2#战\\n士|3#法\\n师|4#道\\n士",
	},
	["prompt"] = { 
		k="prompt",
		value="res/public/btn_npcfh_04.png#5#1#0.6|res/public/btn_npcfh_04.png#10#-7#1",
	},
	["Redtips"] = { 
		k="Redtips",
		value="res/public/btn_npcfh_04.png|res/public/btn_npcfh_03.png",
	},
	["MiniMap"] = { 
		k="MiniMap",
		value="0#0#624#450",
	},
	["AbilityCalMode"] = { 
		k="AbilityCalMode",
		value=0,
	},
	["Heroqiehuan"] = { 
		k="Heroqiehuan",
		value="1#2#3",
	},
	["Heroqiehuanmoshi"] = { 
		k="Heroqiehuanmoshi",
		value=0,
	},
	["itemSacle"] = { 
		k="itemSacle",
		value="0.8|0.8",
	},
	["itemGroundSacle"] = { 
		k="itemGroundSacle",
		value="1|1",
	},
	["Heronuqitiao"] = { 
		k="Heronuqitiao",
		value=1,
	},
	["Fashionexplicit"] = { 
		k="Fashionexplicit",
		value=0,
	},
	["Monsterlevel"] = { 
		k="Monsterlevel",
		value=1,
	},
	["Integratedfashion"] = { 
		k="Integratedfashion",
		value="0#0",
	},
	["heroLoginBtnoffset"] = { 
		k="heroLoginBtnoffset",
		value=0,
	},
	["equipPageNum"] = { 
		k="equipPageNum",
		value=3,
	},
	["EquipAddValueRate"] = { 
		k="EquipAddValueRate",
		value="70#30#5",
	},
	["Floatingblood"] = { 
		k="Floatingblood",
		value="1#99",
	},
	["Floatingblood2"] = { 
		k="Floatingblood2",
		value="100#499",
	},
	["Floatingblood3"] = { 
		k="Floatingblood3",
		value="500#1999",
	},
	["Floatingblood4"] = { 
		k="Floatingblood4",
		value=2000,
	},
	["equipLayerMod"] = { 
		k="equipLayerMod",
		value="0#1#2#3#4#5#6#7#8#60#10#11#16#12#9#61#14#17#18#71#72#73",
	},
	["showShenZhuang"] = { 
		k="showShenZhuang",
		value=0,
	},
	["bagConfig"] = { 
		k="bagConfig",
		value="2#6",
	},
	["SuitCalType"] = { 
		k="SuitCalType",
		value=1,
	},
	["playerLayerPos"] = { 
		k="playerLayerPos",
		value="445#25",
	},
	["bagLayerPos"] = { 
		k="bagLayerPos",
		value="193#62",
	},
	["suitTipColor"] = { 
		k="suitTipColor",
		value="132#132#132",
	},
	["equipStepMes"] = { 
		k="equipStepMes",
		value="1#255#普通物品|2#146#高级物品|3#116#稀世物品|4#244#传说物品|5#244#史诗物品",
	},
	["NPCGoodPriceRate"] = { 
		k="NPCGoodPriceRate",
		value=110,
	},
	["EXPcoordinate"] = { 
		k="EXPcoordinate",
		value="50#200|50#200|250#0|1",
	},
	["AddItemTipsPos"] = { 
		k="AddItemTipsPos",
		value="4|50#200",
	},
	["Team_assembled"] = { 
		k="Team_assembled",
		value=709,
	},
	["EquipItemScale"] = { 
		k="EquipItemScale",
		value=1,
	},
	["isShowAttributeTips"] = { 
		k="isShowAttributeTips",
		value=1,
	},
	["NoAttactBB"] = { 
		k="NoAttactBB",
		value=0,
	},
	["ShowOwnerName"] = { 
		k="ShowOwnerName",
		value=1,
	},
	["AutoMoveRange_NPC"] = { 
		k="AutoMoveRange_NPC",
		value=2,
	},
	["ChangeEquipPageType"] = { 
		k="ChangeEquipPageType",
		value=0,
	},
	["TipsShowIcon"] = { 
		k="TipsShowIcon",
		value=1,
	},
	["missionControl"] = { 
		k="missionControl",
		value=0,
	},
	["value_model"] = { 
		k="value_model",
		value=1,
	},
	["ExpTipsType"] = { 
		k="ExpTipsType",
		value=1,
	},
	["hudFormatShow"] = { 
		k="hudFormatShow",
		value=1,
	},
	["hudFormatShowMonster"] = { 
		k="hudFormatShowMonster",
		value=1,
	},
	["throwNumFormatShow"] = { 
		k="throwNumFormatShow",
		value=1,
	},
	["equipShowTips"] = { 
		k="equipShowTips",
		value=1,
	},
	["attShowTips"] = { 
		k="attShowTips",
		value=1,
	},
	["TipsLowHeight"] = { 
		k="TipsLowHeight",
		value=120,
	},
	["buffShowConfig"] = { 
		k="buffShowConfig",
		value="6#26#-10#-5",
	},
	["clickPressTime"] = { 
		k="clickPressTime",
		value=0.2,
	},
	["pressGoodScale"] = { 
		k="pressGoodScale",
		value="0.9#0.8",
	},
	["skillTipsXY"] = { 
		k="skillTipsXY",
		value="568#240",
	},
	["staticSacle"] = { 
		k="staticSacle",
		value=1,
	},
	["loadingBarExpScale"] = { 
		k="loadingBarExpScale",
		value=0.6,
	},
	["gamePadVisible"] = { 
		k="gamePadVisible",
		value=1,
	},
	["gamePadScale"] = { 
		k="gamePadScale",
		value=0.75,
	},
	["skillSetMode"] = { 
		k="skillSetMode",
		value=2,
	},
	["showMonsterHp"] = { 
		k="showMonsterHp",
		value=0,
	},
	["replaceBasicID"] = { 
		k="replaceBasicID",
		value=1,
	},
	["tombstoneHUD"] = { 
		k="tombstoneHUD",
		value="25#70#90",
	},
	["AutoEnterPkMode"] = { 
		k="AutoEnterPkMode",
		value=0,
	},
	["equipTipsSpName"] = { 
		k="equipTipsSpName",
		value="255#[洗炼属性]|255#[首饰合成]|255#[装备升级]",
	},
	["forceQuickBtn"] = { 
		k="forceQuickBtn",
		value=1,
	},
	["niceboatTimeFormat"] = { 
		k="niceboatTimeFormat",
		value=1,
	},
	["ProtectCheckTime"] = { 
		k="ProtectCheckTime",
		value="1#5",
	},
	["bindTypeStr"] = { 
		k="bindTypeStr",
	},
	["ItemLock"] = { 
		k="ItemLock",
		value=0,
	},
	["hudEnPos"] = { 
		k="hudEnPos",
		value="0#0",
	},
	["MaterialsStdMod"] = { 
		k="MaterialsStdMod",
		value="57#58",
	},
	["sensitiveType"] = { 
		k="sensitiveType",
		value=1,
	},
	["showTargetType"] = { 
		k="showTargetType",
		value=3,
	},
	["color_mask_opcity"] = { 
		k="color_mask_opcity",
		value=240,
	},
	["actor_moving_tick"] = { 
		k="actor_moving_tick",
		value=0.225,
	},
	["actor_moving_pixel_tick2"] = { 
		k="actor_moving_pixel_tick2",
		value=2,
	},
	["comboSkills"] = { 
		k="comboSkills",
		value="烈火剑法#烈火剑法|翔空剑法#翔空剑法|莲月剑法#莲月剑法|烈火剑法#翔空剑法#莲月剑法",
	},
	["ShowNameList"] = { 
		k="ShowNameList",
		value="1#2#3",
	},
	["AutoShieldCD"] = { 
		k="AutoShieldCD",
		value=1000,
	},
	["cytChatTopOff"] = { 
		k="cytChatTopOff",
		value=0,
	},
	["tyxjCloudSpeed"] = { 
		k="tyxjCloudSpeed",
		value="90#30#70",
	},
	["weatherSandstorm"] = { 
		k="weatherSandstorm",
		value="20#50",
	},
	["can_drop_by_storage"] = { 
		k="can_drop_by_storage",
		value=1,
	},
	["AutoFightViewRange"] = { 
		k="AutoFightViewRange",
		value="10#6#6",
	},
	["skillAutoAddNum"] = { 
		k="skillAutoAddNum",
		value="5#12",
	},
	["isShowGuildJobName"] = { 
		k="isShowGuildJobName",
		value=0,
	},
	["NpcTalkUIExitPos"] = { 
		k="NpcTalkUIExitPos",
		value="390#161",
	},
	["EquipDuraTip"] = { 
		k="EquipDuraTip",
		value="1000#2000",
	},
	["EquipDuraNotTip"] = { 
		k="EquipDuraNotTip",
		value="11810#11811#11812",
	},
	["hideMonsterAttrs"] = { 
		k="hideMonsterAttrs",
		value=0,
	},
	["AttackSelectType"] = { 
		k="AttackSelectType",
		value=0,
	},
	["CancelTargetByDoubleClick"] = { 
		k="CancelTargetByDoubleClick",
		value=1,
	},
	["BagSizeToAutoReclaim"] = { 
		k="BagSizeToAutoReclaim",
		value=0,
	},
	["PickEffectState"] = { 
		k="PickEffectState",
		value="0#0",
	},
	["PickerMutex"] = { 
		k="PickerMutex",
		value=1,
	},
	["MapLightEnable"] = { 
		k="MapLightEnable",
		value=0,
	},
	["weatherDarkOpacity"] = { 
		k="weatherDarkOpacity",
		value=180,
	},
	["LoginRoleButtonLow"] = { 
		k="LoginRoleButtonLow",
		value=0,
	},
	["PcItemShourtcutNotMove"] = { 
		k="PcItemShourtcutNotMove",
		value=0,
	},
	["SkillSettingJobShowPage"] = { 
		k="SkillSettingJobShowPage",
		value="8|1#2#3#4#7|5#6#7#8",
	},
	["attrShowInitValue"] = { 
		k="attrShowInitValue",
		value="44#0|65#0",
	},
	["GangColorList"] = { 
		k="GangColorList",
		value="255#3#5#6#19#33#70#94#139#229",
	},
	["ExchangeEquipPageTime"] = { 
		k="ExchangeEquipPageTime",
		value=3,
	},
	["DEFAULT_VSPACE"] = { 
		k="DEFAULT_VSPACE",
		value="4#4",
	},
}
return config
